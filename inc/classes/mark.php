                                ...
                                if(preg_match('/(?<=\s|^)\&gt\;\&gt\;(\d+)(?=\s|$)/', $tokens[$j], $matches) == 1)
                                {
                                        ...
                                        foreach($posts_data as $post_data)
                                                        ...
                                                        && $post_data['post'] == $matches[1])
                                                {
                                                        $found = true;
                                                        $output .= "link:$link_num";
                                                        $links[$link_num++] = "<a class=\"ref|{$post_data['board']}|{$post_data['thread']}|{$post_data['post']}\" href=\"" . KU_BOARDSFOLDER . "/{$post_data['board']}/{$post_data['thread']}#{$post_data['post']}\">{$tokens[$j]}</a>";
                                                        break;
                                                }
                                        if(!$found)
                                        {
...
function lurl_param_handler($content, $argument) {
    ...
    return "class=\"ref|$b|{$thread['original_post']}|$p\" href=\"" . KU_BOARDSFOLDER . "/$b/{$thread['original_post']}#$p\"";
}
    ...
    while (isset($lines[0])) {
        if (preg_match("/^\s*$/", $lines[0])) {
            ...
            while (isset($lines[0]) && preg_match($r1, $lines[0], $m)) {
                ...
                $html .= "<li>" . do_wakabamark($item, $handler, 0) . "</li>";
...
function threads_get_by_reply($b, $p) {
	return 1;
}
