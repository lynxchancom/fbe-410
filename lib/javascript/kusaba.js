...
function QuickReplyForm(replyFormId, replyThreadInputName, quickReplyFormId, quickReplyFormClass, quickReplyFormDetachedClass, quickReplyFormHeaderClass, threadClass, omittedRepliesClass, replyClass, buttonsContainerClass, quickReplyButtonClass) {

  var _quickReplyIcon = '<svg class="icon icon-16"><use xlink:href="/css/icons/sprite.symbol.svg#reply" width="16" height="16" viewBox="0 0 16 16"></use></svg>';
  var _closeFormIcon = '<svg class="icon icon-16"><use xlink:href="/css/icons/sprite.symbol.svg#x" width="16" height="16" viewBox="0 0 16 16"></use></svg>';
  var _detachFormIcon = '<svg class="icon icon-16"><use xlink:href="/css/icons/sprite.symbol.svg#sticky" width="16" height="16" viewBox="0 0 16 16"></use></svg>';

  var _quickReplyHref = '<a href="#" class="post-btn"/>';

  var _quickReplyFormHeader = '<tr><td colspan="2"><a href="#" class="post-badge post-badge-sticky"></a><span></span><a href="#" class="post-btn"></a></td></tr>';
  ...
  function _syncFormTextInputs(sourceForm, targetForm) {
    var syncableInputs = $('input[type=text],textarea', sourceForm);
    ...
    syncableInputs.each(
      function(idx, input) {
        ...
        if (!inputName) {
            ...
            throw Error('Couldnt resolve form input names during form sync setup, aborting init.');
        }
        if (targetForm) {
          var targetFormId = $(targetForm).attr('id') || $(targetForm).find('form').attr('id');
          if (sourceFormId && targetFormId) {
            $(document).on('keyup', '#'+sourceFormId+' [name='+inputName+']', function() { $('[name='+inputName+']', targetForm).val($(input).val());  } );
          }
        } else {
          if (sourceFormId) $(document).off('keyup', '#'+sourceFormId+' [name='+inputName+']');
        }
      }
    );
  }
  ...
  function _quickReply(threadId, postId) {
    if(!_quickReplyForm) {
    ...
    var targetPost = targetThread.find('.'+replyClass+':has(a[name='+postId+'])');
    ...
    if (!_quickReplyForm.hasClass(quickReplyFormDetachedClass)) {
      if ($.scrollTo) {
        $.scrollTo(postId ? targetPost : targetThread, 750, { offset: {top: - $('.topmenu').height() - 3} });
      }
    }
  ...
  function _toggleQuickReplyForm(state) {
    if(!_quickReplyForm) {
    ...
    if (state == 'attach') {
      ...
      if (!_quickReplyForm.is(':hidden')) {
        if ($.scrollTo) {
          $.scrollTo(_quickReplyForm, 750, { offset: {top: - $('.topmenu').height() - 3} });
        }
      }
    }
  ...
  function init() {
    ...
    try {
        ...
        //Setting up bi-directional form content sync
        _syncFormTextInputs(_replyForm, _quickReplyForm);
        _syncFormTextInputs(_quickReplyForm, _replyForm);
  ...
  function deinit() {
    ...
    _syncFormTextInputs(_replyForm, null);
    _syncFormTextInputs(_quickReplyForm, null);
...
$(function(){
   ...
   if (typeof QuickReplyForm !== 'undefined') {
     quickReplyForm = QuickReplyForm('postform', 'replythread', 'quickreplypostform', 'quick-reply-form', 'form-detached', 'replymode', 'thrdcntnr', 'omittedposts', 'reply', 'extrabtns', 'post-btn-reply');
     quickReplyForm.init();
   }
