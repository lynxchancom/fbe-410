...
';
			$items = '';
			$results = $tc_db->GetAll("SELECT * FROM `".KU_DBPREFIX."posts_".$rssboard. "` WHERE `IS_DELETED` = '0' ORDER BY `id` DESC LIMIT 0,30");
			foreach($results AS $row){
				$items .= '<item><title>'.$row['id'].'</title>
<link>';
				if ($row['parentid']!='0') {
				...
				$items .= "</p>]]></description>\n";
				$items .= '<pubDate>' . gmdate("r", $row['postedat']) .'</pubDate>';
				$items .= '<author>' . $row['name'] .'</author>';
				$items .= "</item>\n";
			}
			$items .= '</channel>
			</rss>';
