...
$results = $tc_db->GetAll("SELECT * FROM `".KU_DBPREFIX."boards` WHERE `name` = '".mysqli_real_escape_string($tc_db->link, $_POST['board'])."'");
if (count($results)==0) {
	die();
} else {
...
if (isset($_POST['replyimage'])) {
	if ($_POST['replyimage']!='0') {
		$results = $tc_db->GetAll("SELECT `filename`, `filetype` FROM `".KU_DBPREFIX."posts_".$board_dir."` WHERE `id` = '".mysqli_real_escape_string($tc_db->link, $_POST['replyimage'])."' AND `IS_DELETED` = '0'");
		if (count($results)==0) {
