...
function module_setting_get($module, $key) {
	...
	$query = "SELECT `value` FROM `".KU_DBPREFIX."module_settings` WHERE `module` = '".mysqli_real_escape_string($tc_db->link, $module)."' AND `key` = '".mysqli_real_escape_string($tc_db->link, $key)."' LIMIT 1";
...
function module_setting_set($module, $key, $value, $type = 'string') {
	...
	$exists = $tc_db->GetOne("SELECT COUNT(*) FROM `".KU_DBPREFIX."module_settings` WHERE `module` = '".mysqli_real_escape_string($tc_db->link, $module)."' AND `key` = '".mysqli_real_escape_string($tc_db->link, $key)."' LIMIT 1");
	if ($exists > 0) {
		$result = $tc_db->Execute("UPDATE `".KU_DBPREFIX."module_settings` SET `value` = '".mysqli_real_escape_string($tc_db->link, $value)."') WHERE `module` = '".mysqli_real_escape_string($tc_db->link, $module)."' AND `key` = '".mysqli_real_escape_string($tc_db->link, $key)."'");
	} else {
		$result = $tc_db->Execute("INSERT INTO `".KU_DBPREFIX."module_settings` (`module` , `key` , `value` , `type`) VALUES ('".mysqli_real_escape_string($tc_db->link, $module)."', '".mysqli_real_escape_string($tc_db->link, $key)."', '".mysqli_real_escape_string($tc_db->link, $value)."', '".mysqli_real_escape_string($tc_db->link, $type)."')");
	}
