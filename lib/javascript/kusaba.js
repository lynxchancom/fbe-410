...
function toggleOptions(threadid, formid, board) {
	if (document.getElementById('opt' + threadid)) {
		if (document.getElementById('opt' + threadid).style.display == '') {
			...
			var newhtml = '<td class="label"><label for="formatting">Formatting:</label></td><td colspan="3"><select name="formatting"><option value="" onclick="document.getElementById(\'formattinginfo' + threadid + '\').innerHTML = \'All formatting is performed by the user.\';">Normal</option><option value="aa" onclick="document.getElementById(\'formattinginfo' + threadid + '\').innerHTML = \'[aa] and [/aa] will surround your message.\';"';
			...
			newhtml += '</span></td><td><input type="button" value="Preview" class="submit" onclick="postpreview(\'preview' + threadid + '\', \'' + board + '\', \'' + threadid + '\', document.' + formid + '.message.value);"></td>';
...
function quickreply(threadid) {
	if (threadid == 0) {
		...
		document.getElementById('posttypeindicator').innerHTML = 'reply to ' + threadid + ' [<a href="#postbox" onclick="quickreply(\'0\');" title="Cancel">x</a>]';
	}
...
function hidewatchedthreads() {
	...
	if (document.getElementById('watchedthreads')) {
		document.getElementById('watchedthreads').innerHTML = 'The Watched Threads box will be hidden the next time a page is loaded.  [<a href="#" onclick="showwatchedthreads();return false">undo</a>]';
	}
}
...
function postpreview(divid, board, parentid, message) {
	if (document.getElementById(divid)) {
		new Ajax.Request(ku_boardspath + '/expand.php?preview&board=' + board + '&parentid=' + parentid + '&message=' + escape(message),
		{
			...
			onSuccess: function(transport){
				var response = transport.responseText || "something went wrong (blank response)";
				document.getElementById(divid).innerHTML = unescape(response);
			},
			onFailure: function(){ alert('Something went wrong...') }
		});
	}
...
/* Based on examples from http://javascript.ru/ui/draganddrop thanks a lot to it's author! */
