...
function management_addlogentry($entry, $category = 0, $forceusername = '') {
	...
	$results = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "modlog` WHERE `entry` = '" . mysqli_real_escape_string($tc_db->link, $entry) . "'");
	if (count($results) > 0) {
	...
	if ($entry != '') {
		$tc_db->Execute("INSERT INTO `" . KU_DBPREFIX . "modlog` ( `entry` , `user` , `category` , `timestamp` ) VALUES ( '" . mysqli_real_escape_string($tc_db->link, $entry) . "' , '" . $username . "' , '" . mysqli_real_escape_string($tc_db->link, $category) . "' , '" . time() . "' )");
	}
	if (KU_RSS) {
...
function do_redirect($url, $ispost = false, $file = '') {
	...
	if ($headermethod) {
		header('Location: ' . $url);
		die("If you see this, your browser is not our bro. Do not refresh this page.");
	} else {
		if ($ispost && $file != '') {
