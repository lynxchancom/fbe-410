/* global $, alert, window, document, location: true */
/* global ku_boardspath, delandbanlinks */
...
function highlight(post, checknopage){
   if( checknopage ) return; // do not perform highlights from inline scripts
...
function set_stylesheet(styletitle, txt, site){
   ...
   } else set_cookie('kustyle', styletitle, 365);
...
function togglethread(threadid){
  var arrHidden = JSON.parse(getCookie('hiddenthreads') || '[]');
  ...
  set_cookie('hiddenthreads', JSON.stringify(arrHidden));
  return false;
}
...
function quickreply(threadid){
   if( !threadid ){
     ...
     'reply to ' + threadid +
     ' [<a href="#postbox" onclick="return quickreply(0);" title="Cancel">' +
     'x</a>]'
  );
...
function getwatchedthreads(){
   ...
   /* TODO: undo inline script */ $(function(){
...
function resizeWatchedThreads(){
   ...
   $watched.resizable({
      ...
      stop: function(){
         set_cookie('watchedthreadswidth', $watched[0].offsetWidth, 30);
         set_cookie('watchedthreadsheight', $watched[0].offsetHeight, 30);
      }
   });
}
...
function dragWatchedThreads(){
   ...
   $watched.draggabilly({
      ...
      set_cookie('watchedthreadstop', $watched.css('top'), 30);
      set_cookie('watchedthreadsleft', $watched.css('left'), 30);
   });
}
...
function generateWatchedThreadsElement(){
   ...
   $watched.css({
      top: getCookie('watchedthreadstop') || '185px',
      left: getCookie('watchedthreadsleft') || '25px',
      width: Math.max(250, getCookie('watchedthreadswidth')) + 'px',
      height: Math.max(75, getCookie('watchedthreadsheight')) + 'px'
   });
...
function expandThis(linkEl){
   ...
   var gen3Extensions = ['jpg', 'jpeg', 'png'];
   var videoExtensions = ['mp4', 'webm'];
   var audioExtensions = ['mp3', 'flac', 'ogg'];
      ...
      if( $img[0].src.includes('/thumb/') ){
         ...
         $img.replaceWith(
            $('<img>').addClass('thumb').attr({
               ...
               'background-image': (
                  $.inArray(fullSrcExt, gen3Extensions) < 0 // abandon gen3?
               ) ? 'none' : 'url("' + $link.data('thumbSrc') + '")'
            })
         );
         if(
            $.inArray(fullSrcExt, gen3Extensions) > -1 // using gen3 expander
         ) $link.imagesLoaded().progress(function(il, li){
            $(li.img).css('background-image', 'none');
         });
         // full → thumb:
   ...
   } else return true; // cannot expand a file that has such extension
...
function destroyThumbnailHandlers(){
   $('a.imglink').removeAttr('onclick').prop('onclick', null);
   $('.sound').remove();
}

function resizeThumbnails(){
   destroyThumbnailHandlers();
   $('body').on('click', 'a.imglink', function(){
      return expandThis(this);
   });
}
...
function expandthread(threadid, board){
   ...
   $.ajax({
      ...
      success: function(textHTML){
         ...
         destroyThumbnailHandlers();
         if( typeof delandbanlinks === 'function' ) delandbanlinks();
      }
   });
   return false;
}
...
function textExpandAJAX(){
   $('body').on('click', 'div.abbrev a', function(){
      var $this = $(this);
      ...
      if( $reply.length !== 1 ) return true;

      var matches = /^reply(\d+)$/.exec( $reply[0].id || '' );
...
function setStylesheetUsingCookies(){
   var cookie, title;
   if( style_cookie ){
      cookie = getCookie(style_cookie);
      title = cookie ? cookie : get_preferred_stylesheet();
      set_stylesheet(title);
   } else if( style_cookie_txt ){
      cookie = getCookie(style_cookie_txt);
      title = cookie ? cookie : get_preferred_stylesheet();
      set_stylesheet(title, true);
   } else if( style_cookie_site ){
      cookie = getCookie(style_cookie_site);
      title = cookie ? cookie : get_preferred_stylesheet();
...
var resizeMaster = {
   // there is a content script that starts with `request_faptcha(`
   // and expects resizeMaster.setResizer to be a function
   setResizer: function(){}
};

$(function(){
   setStylesheetUsingCookies();
   hideHiddenThreads();
   textExpandAJAX();
   addPreviewEvents();
   ...
   // if( getCookie('showwatchedthreads') ) generateWatchedThreadsElement();
   resizeWatchedThreads();
   dragWatchedThreads();
   ...
   $(window).on('unload', function(){
      if( style_cookie ){
         ...
         set_cookie(style_cookie_txt, get_active_stylesheet(), 365);
      } else if( style_cookie_site ) {
         //do nothing
      }
   });
